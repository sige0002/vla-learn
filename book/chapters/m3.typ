...
#fig("/figures/normalization.png", caption: [本教材の実データで見るスケール差。左: 生の行動は
  `dx`/`dy`（$plus.minus 0.08$、両端のスパイクはクリップ）と `grip_cmd`（0/1）で桁が違う。
  右: 正規化後はどの次元も同じ土俵に乗る。生成は #raw("scripts/make_figures.py")。], width: 100%)
