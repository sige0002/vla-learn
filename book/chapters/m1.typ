...
- *出力サイズの公式（これだけ暗記）*: `out = (in + 2*padding - kernel) // stride + 1`。
  上の例は $(64 + 2 - 3) "//" 2 + 1 = 32$。`stride=2` は窓を 1 つ飛ばしで当てるので *空間サイズがほぼ半分* になります。
- *flatten*: 畳み込みの出力 `[B, 64, 4, 4]` を全結合に入れるときは `x.flatten(1)` で
  `[B, 64*4*4] = [B, 1024]` に伸ばします（先頭のバッチ次元は残す）。

本書の `ImageEncoder`（M4）は、この Conv2d（stride=2）を 4 回重ねて $64 -> 32 -> 16 -> 8 -> 4$ と縮め、最後に
flatten します。「stride=2 で毎回半分」と出力サイズの公式さえ手で言えれば、M2 以降の画像まわりの shape は
全部自力で追えます。
...
#readcode("src/vla_learn/training/checkpoint.py", target: "save_policy / load_policy")[
  この定石の実物です。`state_dict` + モデル種別 + コンストラクタ引数 + トークナイザ語彙 + 正規化統計を
  1 つの `policy.pt` にまとめ、`load_policy` がファイル 1 個から方策を完全に復元します（M4 で使います）。
]
