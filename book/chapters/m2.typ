...
#fig("/figures/covariate_shift.png", caption: [左: 教師デモの「通り道」（青の帯）から小さな誤差で外れると、
  教師データの無い領域に入り誤差が複利で増える（模式図）。右: 対策の効き方のイメージ — ノイズ注入データ（M2 後半）は
  「外れたときの戻り方」を教え、receding horizon（M3〜）は誤差が育つ前に観測し直す。
...
#fig("/figures/loss_curve.png", caption: [TinyVLA の学習曲線（全データを使い、ミニバッチごとの損失を
  ステップ順に並べたもの）。下がっても *閉ループで解けるとは限らない* のが本章の教訓。
  図は #raw("scripts/make_figures.py") の #raw("fig_loss_and_rollout")。], width: 80%)
