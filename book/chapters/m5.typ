...
#fig("/figures/flow_path.png", caption: [左: rectified flow の直線経路 $a_tau=(1-tau)a_0+tau a_1$。多数のノイズ $a_0$ から
  目標 $a_1$ へ各々まっすぐ向かう（速度 $v^*=a_1-a_0$ は経路上一定）。右: 学習後に Euler 積分でノイズ 1 点から目標へ歩く様子。
  `flow_steps` を増やすと刻みが細かくなる。生成は #raw("scripts/make_figures.py")。], width: 100%)
...
#note[
  理想的な速度場なら 1 歩でも直線で着きますが、学習した *近似* 速度場では刻み数が効きます（前掲図の右パネルで
  `flow_steps` を変えた軌跡の粗密がこれにあたります）。
]
