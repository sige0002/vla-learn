...
#theory[
  ...
  *他の全位置をどれだけ参照するかの重みを学習して情報を混ぜる*」仕組みです。「赤」の位置のベクトルが
  ...
  `batch_first=True`=入出力が `[B, L, D]` 順。`src_key_padding_mask` は *True の位置を「見ない」* ための
  マスクです（PAD を文脈に混ぜない）。
]
...
=== `policy.pt` の中身 — ブラックボックスにしない

`[save]` で書かれる `policy.pt` は魔法のファイルではなく、M1 で学んだ *state_dict の定石* そのものです。
`save_policy` は 1 つの辞書を `torch.save` しているだけ — 中身は `model_state`（重み）に加えて、
`model_type`（どのクラスで作り直すか）・`model_kwargs`（コンストラクタ引数）・トークナイザ語彙・
正規化統計・`chunk_len`。`load_policy` は逆順に「クラスを選ぶ → 構築 → `load_state_dict` → eval モード」です。

#readcode("src/vla_learn/training/checkpoint.py", target: "save_policy / load_policy")[
  *重みだけでなくトークナイザ語彙と正規化統計まで保存する* のがポイント。どれか 1 つ欠けても
  「学習時と同じ前処理」が再現できず、方策は静かに壊れます。ファイル 1 個で完全復元、を確認してください。
]
...
#note[
  補足 — この成功率は「汎化」を測っています: `evaluate_policy` の評価エピソードは `seed=10_000+i` で生成され、
  学習データ（別の seed 系列）には *出てこない初期配置* です。つまり `success_rate` は訓練の再生ではなく汎化性能。
  より強い汎化テストは、学習で見ていない指示テンプレや色の組（OOD）で測ることです（M6 卒業課題）。もし
  「学習データの配置では成功、新配置で大きく落ちる」なら過学習（暗記）を疑います。
]
