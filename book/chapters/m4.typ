...
#fig("/figures/architecture.png", caption: [TinyVLA / FlowVLA の forward パイプライン。3 入力（画像・トークン・状態）を
  ...
  右端のヘッドだけが M4（MSE 回帰）と M5（flow）で differ。実装は #raw("src/vla_learn/models/tiny_vla.py")。], width: 100%)
...
（言語と対象の対応付け）が原理的に不能になります。そこで `TextEncoder` は *位置埋め込み + 1 層の Transformer
エンコーダ* で語順を考慮します:
...
#fig("/figures/loss_curve.png", caption: [全データでの学習曲線（縦軸 log）。MSE は単調に近く下がる。M5 の flow 損失は
...
#fig("/figures/rollout_filmstrip.png", caption: [学習した TinyVLA のロールアウト連続フレーム。白がグリッパ。指示色の
  ブロックを指示色のゴールへ運ぶ（この例は失敗ケース）。`success_rate` は多数エピソードの平均。実装は
