...
#set text(size: 8.5pt)
#table(
  columns: (1.1fr, 0.9fr, 1.25fr, 1.25fr, 0.55fr, 0.75fr),
...
VLA 以前の系譜も 2 つだけ押さえます（表の「前史」行）。*Diffusion Policy*（Chi et al. 2023）は「行動チャンクを
diffusion で生成する」ことを確立した原典で、*M5 の flow ヘッドの直接の先祖* です（言語なし・視覚+状態のみ。
あなたの diffusion 座学に最も近い実装）。*ACT*（Zhao et al. 2023, ALOHA）は「行動チャンク + temporal ensembling +
CVAE」でテレオペ模倣を実用にした代表作で、本書の `chunk_len` や M4 の ensembling の出所です。また OpenVLA の
2025 改良版 *OpenVLA-OFT* は離散自己回帰を「並列デコード + 連続値の L1 回帰」に置き換えて推論を大幅高速化
しました — 離散 vs 連続の議論が今も動いている好例です。

3 系統のトレードオフを一言ずつ: *連続回帰（M4）* は最速・最簡だが多峰を平均で潰す。*離散トークン（OpenVLA 系）* は
行動を「単語」にするので *言語モデルの機構（語彙・自己回帰・尤度）をそのまま使える* のが最大の利点だが、自己回帰の
ぶん推論が遅く、ビン離散化の量子化誤差が乗る。*flow / diffusion（M5, π0 系）* は多峰を保ったまま連続値を生成できるが、
積分ステップのぶん推論が重い。この 3 択の感触は #raw("exercises/m6/discrete_head.py")（256 ビン離散化ヘッドを自作して
MSE 版・flow 版と比べる演習）で手を動かして確かめられます。
...
=== 実 VLA の使い方は「fine-tune」— 本書との最大の運用差

本書は終始スクラッチ学習でしたが、実務で OpenVLA や π0 系を新しいロボット・新タスクに使うときは、
*事前学習済みチェックポイントを fine-tune する* のが標準です。押さえる概念は 3 つだけ:

- *どこを凍結し、どこを学習するか*: 定番は「VLM（視覚・言語）側は凍結または低学習率、action head（expert）は
  付け替えて学習」。行動空間（関節数・単位系）はロボットごとに違うので head の付け替えはほぼ必須です —
  あなたが M4→M5 でやった「*head 差し替え*」の実務版がこれです。
- *LoRA（低ランク適応）*: 7B 級 VLM の全重みを更新する代わりに、attention 層などに小さな低ランク行列を挿して
  *そこだけ学習* します。OpenVLA の公式 fine-tune も LoRA（rank 32）が既定。座学で知る LoRA の適用先が
  「VLM の attention 層 + action head」だと分かっていれば、実 repo の設定ファイルが読めます。
- *データ効率と co-training*: 新タスクのデモは数十〜数百本のことが多く、事前学習で使った広い分布のデータと
  混ぜて学習（co-training）して忘却を防ぎます（π0.5 の主要テーマ）。

#note[
  *推論効率の話も 1 分だけ*: 実機の制御は 10〜50Hz で回るのに、7B 級 VLA の 1 推論は数十〜数百 ms かかります。
  対策の軸は (1) *action chunking* で推論回数を減らす（M3 で学んだ理由の実務版）、(2) 自己回帰系は
  *KV キャッシュ*（過去トークンの attention 計算を使い回す）や *並列デコード*（OpenVLA-OFT）、
  (3) *重みの量子化*（8/4bit 化。本書で出た「行動の離散化」とは別物）や蒸留です。
]

#note[
  *卒業課題の思考実験*: 本書の `VLABackbone` を凍結して `head` だけ再学習したら、成功率はどこまで戻るか？
...
本書は「実機なし」で完結しますが、その先（LeRobot の実機チュートリアル等）へ進むときに *新たに増えるもの* を
正直に列挙しておきます。逆に言えば、これ以外（モデル・損失・データ形式・評価の考え方）は本書で学んだままです:
...
+ *（発展）temporal ensembling を実装*する（M4 の「チャンク境界の段差」の節）。
  到達目標: 毎ステップ予測 + 過去チャンクの指数重み平均 $w_i prop exp(-m dot i)$ を `PolicyWrapper` の外側の
  ラッパとして書ける。
  評価: 素朴な receding horizon と成功率・軌道の滑らかさ（隣接行動の差のノルム）を比較する。
