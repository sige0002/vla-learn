...
```bash
# 0) リポジトリのルートに移動（このファイルの 2 つ上の階層）
cd vla_learn

# 1) 仮想環境 (venv) を作って有効化する
python3 -m venv .venv
source .venv/bin/activate        # Windows は .venv\Scripts\activate

# 2) PyTorch（CPU 版）を入れる
pip install torch --index-url https://download.pytorch.org/whl/cpu

# 3) 本パッケージを editable install（vla_learn を import できるようにする）
pip install -e .

# 任意) 可視化(matplotlib)とテスト(pytest)も入れたい場合
pip install -e ".[viz,dev]"
```

> **`pip install -e .` とは**: カレントの `pyproject.toml` を読み、`src/vla_learn` を
> 「編集可能 (editable)」な状態で import できるようにします。これで Python のどこからでも
> `import vla_learn` が通り、`src/` を書き換えればすぐ反映されます。
> （install したくない場合は、`scripts/` の各ファイル冒頭にある `import _bootstrap` が
> `src/` をパスに追加してくれるので、`PYTHONPATH=src` 相当でも動きます。）
...
> **困ったとき**
> - `ModuleNotFoundError: No module named 'vla_learn'` → 仮想環境を有効化したか、`pip install -e .` を実行したか確認。
> - `ModuleNotFoundError: No module named 'torch'` → 手順 2 の PyTorch インストールが未実行です。
> - スクリプトを直接 `python scripts/...` で動かしてください（`import _bootstrap` が `src/` をパスに通します）。
...
- 環境構築は `venv` → `pip install torch`(CPU) → `pip install -e .` → `pytest` → `configs/smoke.json` で学習が回る確認、まで。
