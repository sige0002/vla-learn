...
> 注: これは M2 用の**最小 CNN**です。M4 の `ImageEncoder`（`../../src/vla_learn/models/image_encoder.py`）は
> 畳み込みを **4 段**（`64→32→16→8→4`）にしてチャンネルも `64` まで増やすため、Flatten 後は `64*4*4=1024`
> になります（段数・次元が違う別物）。M2 の `2048` と混同しないでください。
